...
const $ = s => {
  ...
  function toggleMode() {
    ...
    const boxes = document.querySelectorAll('.homework-list, .transparent-box, header, .sidebar, footer'); // 包含footer

    // 切换为 Dark 模式
    if (!body.classList.contains('dark-mode')) {
        body.classList.add('dark-mode');
        components.classList.add('active'); // 触发切换动画

        // 修改所有半透明框
        boxes.forEach(box => {
            box.style.backgroundColor = 'rgba(0, 0, 0, 0.8)'; // 黑色半透明
            box.style.color = 'white'; // 白色文字
        });

        // 修改标题栏和侧边栏样式
        header.style.backgroundColor = 'rgba(0, 0, 0, 0.8)'; // 黑色半透明
        header.style.color = 'white'; // 白色文字
        sidebar.style.backgroundColor = 'rgba(0, 0, 0, 0.8)'; // 黑色半透明
        sidebar.style.color = 'white'; // 白色文字
        ...
        // 修改所有半透明框
        boxes.forEach(box => {
            box.style.backgroundColor = 'rgba(255, 255, 255, 0.2)'; // 白色半透明
            box.style.color = 'black'; // 黑色文字
        });

        // 修改标题栏和侧边栏样式
        header.style.backgroundColor = 'rgba(255, 255, 255, 0.2)'; // 白色半透明
        header.style.color = 'black'; // 黑色文字
        sidebar.style.backgroundColor = 'rgba(255, 255, 255, 0.2)'; // 白色半透明
        sidebar.style.color = 'black'; // 黑色文字
