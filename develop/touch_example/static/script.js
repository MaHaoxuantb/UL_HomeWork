...
// 标记作业为完成
document.getElementById('completeAssignmentForm').addEventListener('submit', function(event) {
    event.preventDefault();
    const studentId = document.getElementById('completeStudentId').value;
    const classId = document.getElementById('completeClassId').value;
    const assignmentId = document.getElementById('completeAssignmentId').value;

    fetch('http://127.0.0.1:8000/complete_assignment', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${token}`
